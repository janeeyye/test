...
function renderSidebar(){
  if (highlightListEl){
    ...
    highlights.slice(0, 2).forEach(item => {
      ...
      card.style.borderLeftColor = SOLUTION_COLOR[item.solution] || "var(--muted-foreground)";
  ...
  if (onDemandListEl){
    ...
    onDemand.slice(0, 3).forEach(item => {
      const a = document.createElement("a");
      ...
      date.textContent = item.date ? `${item.date} 진행` : "On-demand";
  ...
  if (quickLinkListEl){
    ...
    quickLinks.slice(0, 2).forEach(item => {
